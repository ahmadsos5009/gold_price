...
const ChartsPage: React.FC<any> = ({ dates, data, price, previousPrice }) => {
  ...
  useEffect(() => {
    ...
    setDataLine({
      labels: dates.map((timestamp: number) => new Date(timestamp * 1000).getDate()),
      datasets: [
        {
          label: "Price History",
          fill: true,
          ...
          pointRadius: 1,
          pointHitRadius: 10,
          data: data.map((price: number) => price.toFixed(2)),
        },
      ],
    });
