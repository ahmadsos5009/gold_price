...
const PriceTable: React.FC<PriceTable> = ({price, countryCurrency, updateDate}) => {
   return (
    ...
    <Table striped bordered hover responsive>
      <thead>
        <tr>
          <th colSpan={2}>Price table based on purity, In {countryCurrency}</th>
        </tr>
      </thead>
      <tbody>
        <tr>
        ...
        <tr>
          <td>18K</td>
          <td>{calculate_price_per_unit(price, 0.750)}</td>
        </tr>
      </tbody>
      <caption><Badge variant="info">latest update of price {updateDate.toUTCString()} </Badge></caption>
    </Table>
  </React.Fragment>
   );
}
